                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="student_id" id="student_id" required 
                                class="w-full bg-gray-700 border border-gray-600 rounded-lg px-4 py-3 text-white focus:ring-2 focus:ring-purple-500 focus:border-transparent @error('student_id') border-red-500 @enderror">
                            ...
                            @foreach($students as $student)
                                <option value="{{ $student->id }}" 
                                    ...
                                    {{ $student->name }} ({{ $student->student_id }}) - {{ $student->sport->name ?? 'No Sport' }}
                                </option>
                            @endforeach
                        </select>
                        @error('student_id')
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Auto-select batch when student is selected
    studentSelect.addEventListener('change', function() {
        const selectedOption = this.options[this.selectedIndex];
        const batchId = selectedOption.getAttribute('data-batch');
    ...
    // Load student info if pre-selected
    if (studentSelect.value) {
        loadStudentInfo(studentSelect.value);
    }
});
</script>
@endsection