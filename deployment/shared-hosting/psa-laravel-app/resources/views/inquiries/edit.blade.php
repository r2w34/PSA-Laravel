...
@section('page-description', 'Update inquiry information and status')
    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold text-white">Edit Inquiry</h1>
                    <p class="mt-2 text-gray-400">Update inquiry information and status</p>
                </div>
        ...
        <div class="bg-gray-800 rounded-xl border border-gray-700 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-700">
                ...
                <p class="text-sm text-gray-400 mt-1">Update the inquiry details and status</p>
            </div>

            <form method="POST" action="{{ route('inquiries.update', $inquiry) }}" class="p-6 space-y-6">
                @csrf
                @method('PUT')
                        ...
                        <div>
                            ...
                            <select id="sport_id" 
                                ...
                                @foreach($sports as $sport)
                                    <option value="{{ $sport->id }}" {{ old('sport_id', $inquiry->sport_id) == $sport->id ? 'selected' : '' }}>
                                        {{ $sport->name }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select id="batch_id" 
                                ...
                                @foreach($batches as $batch)
                                    <option value="{{ $batch->id }}" 
                                            ...
                                            {{ old('batch_id', $inquiry->batch_id) == $batch->id ? 'selected' : '' }}
                                            style="display: none;">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select id="status" 
                                    name="status" 
                                    required
                                    class="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white focus:ring-2 focus:ring-purple-500 focus:border-transparent @error('status') border-red-500 @enderror">
                                <option value="">Select Status</option>
                                <option value="new" {{ old('status', $inquiry->status) == 'new' ? 'selected' : '' }}>New</option>
                                <option value="in_progress" {{ old('status', $inquiry->status) == 'in_progress' ? 'selected' : '' }}>In Progress</option>
                        ...
                        <div>
                            ...
                            <select id="source" 
                                ...
                                <option value="phone" {{ old('source', $inquiry->source) == 'phone' ? 'selected' : '' }}>Phone Call</option>
                                ...
                                <option value="advertisement" {{ old('source', $inquiry->source) == 'advertisement' ? 'selected' : '' }}>Advertisement</option>
                            </select>
                            @error('source')
                                <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="date" 
                                   id="follow_up_date" 
                                   name="follow_up_date" 
                                   value="{{ old('follow_up_date', $inquiry->follow_up_date ? $inquiry->follow_up_date->format('Y-m-d') : '') }}"
                ...
                <!-- Student Conversion (if status is converted) -->
                @if($inquiry->status === 'converted' || request('convert'))
                <div class="border-t border-gray-700 pt-6">
                    <h4 class="text-lg font-medium text-white mb-4">Student Conversion</h4>
                    
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label for="student_id" class="block text-sm font-medium text-gray-300 mb-2">Link to Student</label>
                            <select id="student_id" 
                                    name="student_id" 
                                    class="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white focus:ring-2 focus:ring-purple-500 focus:border-transparent @error('student_id') border-red-500 @enderror">
                                <option value="">Select Student (Optional)</option>
                                @foreach($students as $student)
                                    <option value="{{ $student->id }}" {{ old('student_id', $inquiry->student_id) == $student->id ? 'selected' : '' }}>
                ...
                <div class="border-t border-gray-700 pt-6">
                    <div>
                        ...
                        <textarea id="notes" 
                                  ...
                                  placeholder="Add any additional notes about this inquiry...">{{ old('notes', $inquiry->notes) }}</textarea>
                        @error('notes')
                            <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
