        ...
        <form action="{{ route('payments.update', $payment) }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select id="batch_id" 
                        ...
                        <option value="{{ $batch->id }}" {{ (old('batch_id', $payment->batch_id) == $batch->id) ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select id="payment_type" 
                        ...
                        <option value="monthly" {{ old('payment_type', $payment->payment_type) == 'monthly' ? 'selected' : '' }}>Monthly Fee</option>
                        <option value="registration" {{ old('payment_type', $payment->payment_type) == 'registration' ? 'selected' : '' }}>Registration Fee</option>
                        <option value="tournament" {{ old('payment_type', $payment->payment_type) == 'tournament' ? 'selected' : '' }}>Tournament Fee</option>
                    </select>
                    @error('payment_type')
                    <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="datetime-local" 
                           id="due_date" 
                           name="due_date" 
                           value="{{ old('due_date', $payment->due_date ? $payment->due_date->format('Y-m-d\TH:i') : '') }}"
                ...
                <div>
                    ...
                    <input type="text" 
                           id="month_year" 
                           name="month_year" 
                           value="{{ old('month_year', $payment->month_year) }}"
            ...
            <div>
                ...
                <textarea id="notes" 
                          ...
                          class="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent @error('notes') border-red-500 @enderror">{{ old('notes', $payment->notes) }}</textarea>
                @error('notes')
                <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                @enderror
            </div>
